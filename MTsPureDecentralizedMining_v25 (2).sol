...
contract MTsPureDecentralizedMining is ERC721, Ownable, ReentrancyGuard {
    ...
    function poolHealth() external view returns (
        ...
        uint256 realBal = mtsToken.balanceOf(address(this));
        uint256 accBal  = accountedBalance;
        uint256 res     = rewardReserve;
        uint256 cry     = totalCrystallizedRewards;
        uint256 uncry   = totalUncrystallizedRewards();
        uint256 liab    = res + cry + uncry;
        uint256 rate    = getCurrentEmissionRate();

        uint256 pend = 0;
        if (totalGlobalHashPower > 0 && res > 0 && block.timestamp > lastUpdateTime) {
            pend = _getCappedEmission(
                lastUpdateTime,
                block.timestamp,
                res,
                cry,
